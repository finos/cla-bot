...
const sideEffect = fn => d => fn(d).then(() => d);
...
exports.handler = ({ body }, lambdaContext, callback) => {
  ...
  if (!validAction(body)) {
    callback(
      null,
      response({ message: `ignored action of type ${body.action}` })
    );
    return;
  }
  ...
  const loggingCallback = (error, message) => {
    ...
    logger.flush(context.logUrl).then(() => callback(error, response(message)));
  };
  ...
  if (body.action === "created") {
    if (!commentSummonsBot(body.comment.body)) {
      ...
      loggingCallback(null, {
        message: "the comment didnt summon the cla-bot"
      });
      return;
    } else {
      if (body.comment.user.login === `${process.env.BOT_NAME}[bot]`) {
        loggingCallback(null, {
          message: "the cla-bot summoned itself. Ignored!"
        });
        return;
      }
  ...
  logger.info(
    `Checking CLAs for pull request ${context.gitHubUrls.pullRequest}`
  );
  ...
  Promise.resolve()
    ...
    .then(nonContributors => {
      if (nonContributors.length === 0) {
        ...
        return githubRequest(getLabels(context), context.userToken, "GET")
          .then(labels => {
            ...
            if (!labels.some(l => l.name === context.config.label)) {
              ...
              logger.info(
                `The pull request already has the label ${context.config.label}`
              );
            }
          })
          .then(() =>
            githubRequest(setStatus(context, "success"), context.userToken)
          )
          .then(
            () =>
              `added label ${context.config.label} to ${
                context.gitHubUrls.pullRequest
              }`
          );
      } else {
        ...
        return githubRequest(
          addComment(context, usersWithoutCLA),
          context.userToken
        )
          .then(() => githubRequest(deleteLabel(context), context.userToken))
          .then(() =>
            githubRequest(setStatus(context, "error"), context.userToken)
          )
          .then(
            () =>
              `CLA has not been signed by users ${usersWithoutCLA}, added a comment to ${
                context.gitHubUrls.pullRequest
              }`
          );
      }
    ...
    .catch(err => {
      ...
      githubRequest(setStatus(context, "failure"), context.userToken).then(() =>
